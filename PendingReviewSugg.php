...
if (isset($_SESSION['languages'])) {
    ...
    $query      = "SELECT * FROM suggestions WHERE language='$languagess' AND (status='Pending Review') ORDER BY term ASC";
} else {
    $query = "SELECT * FROM suggestions WHERE status='Pending Review' ORDER BY term ASC";
}
include('headerforsuggestion.php');
//add the id in the suggestions table
$ids    = "";
...
<th>More</th>
    ...
    echo "<table width='100%' bgcolor='#FFFFFF' height='82' border='1' class='table' style='border-collapse:collapse' align='left'>";
    echo "<tr><td>No Suggestions in Pending Review state</a></td></tr>";
    echo "</table>";
}
include("footer.php");
?>